...
```
host/                     # Go 程序（Fyne 桌面 GUI，module: github.com/local/esp32-ai-usage-host）
...
go build -ldflags "-s -w -H=windowsgui" -o bin/esp32-ai-usage.exe .   # 或直接跑 build.bat（-s -w 瘦身约 43→24MB）
go vet ./...
go run .                               # 开发期直接跑（会弹界面）
